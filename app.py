...
v = st.sidebar.selectbox('Select a chart type : ',('Bar chart','Pie chart'))
stt = st.sidebar.selectbox('Select a State : ',df1['State'].unique())
status = st.sidebar.radio('Patient Status : ',('Confirmed cases','Recovered cases','Active cases','Death cases'))
state = df1[df1['State'] == stt]
...
if v == 'Bar chart':
    ...
    if status == 'Confirmed cases':
        ...
        fig = px.pie(df1,values=df1['Confirmed'],names=df1['State'])
        st.plotly_chart(fig)
    elif status == 'Recovered cases':
        st.title("Total Recovered cases ")
        fig = px.pie(df1,values=df1['Recovered'],names=df1['State'])
        st.plotly_chart(fig)
    elif status == 'Active cases':
        st.title("Total Active cases ")
        fig = px.pie(df1,values=df1['Active'],names=df1['State'])
        st.plotly_chart(fig)
    else:
        st.title("Total Death cases ")
        fig = px.pie(df1,values=df1['Deaths'],names=df1['State'])
        st.plotly_chart(fig)
